  ...
  class Maintenance
    ...
    def apply_certificate_update_endpoint(server_resp, check_for_renew_request = true)
      ...
      if cert_update_endpoint.empty?
        ...
        log_error("Could not find the updateCertificate tag in the AgentService heartbeat response")
    ...
    def heartbeat
      ...
      begin
        ...
        if !xml_contains_telemetry(body_hb_xml)
          log_debug("No Telemetry data was appended to the AgentService heartbeat request")
        end
      rescue => e
        log_error("Error when appending Telemetry to AgentService Heartbeat: #{e.message}")
      end
      ...
      if !res.nil?
        log_info("AgentService Heartbeat response code: #{res.code}") if @verbose
        ...
        if res.code == "200"
          ...
          if cert_apply_res.class != String
            ...
            log_info("AgentService Heartbeat success")
            return 0
          end
        else
          log_error("Error sending the AgentService heartbeat. HTTP code #{res.code}")
          return HTTP_NON_200
        end
      else
        log_error("Error sending the AgentService heartbeat. No HTTP code")
